    ...
    <el-table v-loading="loading" :data="studentList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="爱好" align="center" prop="studentHobby">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.student_hobby" :value="scope.row.studentHobby ? scope.row.studentHobby.split(',') : []"/>
        </template>
      </el-table-column>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="爱好">
          <el-checkbox-group v-model="form.studentHobby">
            <el-checkbox
              v-for="dict in dict.type.student_hobby"
              :key="dict.value"
              :label="dict.value">
              {{dict.label}}
            </el-checkbox>
...
<script>
  ...
  export default {
    name: "Student",
    dicts: ['sys_user_sex', 'student_hobby'],
    data() {
      return {
        // 遮罩层
        loading: true,
